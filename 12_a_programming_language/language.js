...
topScope.length = args => {
    ...
    throw new ReferenceError("object should be an array");
};

topScope.element = (args, idx) => {
    if (Array.isArray(args)) {
        return args[idx];
    }
    throw new ReferenceError("object should be an array");
...
function commentsTest1() {
    console.log(parse("### hello\nx"));
// → {type: "word", name: "x"}
}

function commentsTest2() {
...
specialForms.set = (args, scope) => {
    ...
    if (args[0].type !== "word") {
        throw new SyntaxError("First arg must be word");
    }
    ...
    throw new ReferenceError(`Variable with a name ${name} wasn't found`);
};
...
let fixingScope2 = `set(quux, true)`;
